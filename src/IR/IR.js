...
const jsFuncMap = {
  'saw': {"setup":(o,p)=>`${o} = new Module.maxiOsc(); ${o}.phaseReset(${p.length>1 ? p[1].loop : 0.0});`, 
          "loop":(o,p)=>`${o}.saw(${p[0].loop})`},
  'sin': {"setup":(o,p)=>`${o} = new Module.maxiOsc(); ${o}.phaseReset(${p.length>1 ? p[1].loop : 0.0});`, 
          "loop":(o,p)=>`${o}.sinewave(${p[0].loop})`},
  'tri': {"setup":(o,p)=>`${o} = new Module.maxiOsc(); ${o}.phaseReset(${p.length>1 ? p[1].loop : 0.0});`, 
          "loop":(o,p)=>`${o}.triangle(${p[0].loop})`},
  'pha': {"setup":(o,p)=>`${o} = new Module.maxiOsc(); ${o}.phaseReset(${p.length>1 ? p[1].loop : 0.0});`, 
          "loop":(o,p)=>`${o}.phasor(${p[0].loop})`},
  'ph2': {"setup":(o,p)=>`${o} = new Module.maxiOsc(); ${o}.phaseReset(${p.length>3 ? p[3].loop : 0.0});`, 
          "loop":(o,p)=>`${o}.phasor(${p[0].loop},${p[1].loop},${p[2].loop})`},
  'sqr': {"setup":(o,p)=>`${o} = new Module.maxiOsc(); ${o}.phaseReset(${p.length>1 ? p[1].loop : 0.0});`, 
          "loop":(o,p)=>`${o}.square(${p[0].loop})`},
  'pul': {"setup":(o,p)=>`${o} = new Module.maxiOsc(); ${o}.phaseReset(${p.length>2 ? p[2].loop : 0.0});`, 
          "loop":(o,p)=>`${o}.pulse(${p[0].loop},${p[1].loop})`},
  'imp': {"setup":(o,p)=>`${o} = new Module.maxiOsc(); ${o}.phaseReset(${p.length>1 ? p[1].loop : 0.0});`, 
          "loop":(o,p)=>`${o}.impulse(${p[0].loop})`},
  'sawn': {"setup":(o,p)=>`${o} = new Module.maxiOsc(); ${o}.phaseReset(${p.length>1 ? p[1].loop : 0.0});`, 
           "loop":(o,p)=>`${o}.sawn(${p[0].loop})`},
  'noiz': {"setup":(o,p)=>`${o} = new Module.maxiOsc()`, 
  ...
  'env': {"setup":(o,p)=>`${o} = new Module.maxiEnv();${o}.setAttack(${p[1].loop});${o}.setDecay(${p[2].loop});${o}.setSustain(${p[3].loop});${o}.setRelease(${p[4].loop})`, 
          "loop":(o,p)=>`${o}.adsr(1,${p[0].loop})`},
  'sum': {"setup":(o,p)=>"", 
          "loop":(o,p)=>{let s=`(${p[0].loop}`; for(let i=1; i < p.length; i++) s += `+${p[i].loop}`; return s+")";}},
  'mix': {"setup":(o,p)=>"", 
          "loop":(o,p)=>{let s=`((${p[0].loop}`; for(let i=1; i < p.length; i++) s += `+${p[i].loop}`; return s+`)/${p.length})`;}},
  'prod': {"setup":(o,p)=>"", 
           "loop":(o,p)=>{let s=`(${p[0].loop}`; for(let i=1; i < p.length; i++) s += `*${p[i].loop}`; return s+")";}},
  'blin': {"setup":(o,p)=>"", 
  ...
  'dist': {"setup":(o,p)=>`${o} = new Module.maxiDistortion()`, 
           "loop":(o,p)=>`${o}.atanDist(${p[0].loop},${p[1].loop})`},
  'flange': {"setup":(o,p)=>`${o} = new Module.maxiFlanger()`, 
             "loop":(o,p)=>`${o}.flange(${p[0].loop},${p[1].loop},${p[2].loop},${p[3].loop},${p[4].loop})`},
  'chor': {"setup":(o,p)=>`${o} = new Module.maxiChorus()`, 
           "loop":(o,p)=>`${o}.chorus(${p[0].loop},${p[1].loop},${p[2].loop},${p[3].loop},${p[4].loop})`},
  'dl': {"setup":(o,p)=>`${o} = new Module.maxiDelayline()`, 
         "loop":(o,p)=>`${o}.dl(${p[0].loop},${p[1].loop},${p[2].loop})`},
  'lpf': {"setup":(o,p)=>`${o} = new Module.maxiFilter()`, 
          "loop":(o,p)=>`${o}.lopass(${p[0].loop},${p[1].loop})`},
  'hpf': {"setup":(o,p)=>`${o} = new Module.maxiFilter()`, 
          "loop":(o,p)=>`${o}.hipass(${p[0].loop},${p[1].loop})`},
  'lpz': {"setup":(o,p)=>`${o} = new Module.maxiFilter()`, 
          "loop":(o,p)=>`${o}.lores(${p[0].loop},${p[1].loop},${p[2].loop})`},
  'hpz': {"setup":(o,p)=>`${o} = new Module.maxiFilter()`, 
          "loop":(o,p)=>`${o}.hires(${p[0].loop},${p[1].loop},${p[2].loop})`},
  'toJS': {"setup":(o,p)=>`${o} = this.registerTransducer('${o}', ${p[0].loop})`, 
           "loop":(o,p)=>`${o}.send(${p[1].loop}, ${p[2].loop})`},
  'fromJS': {"setup":(o,p)=>`${o} = this.registerTransducer('${o}', ${p[0].loop})`, 
             "loop":(o,p)=>`${o}.receive(${p[1].loop})`},
  'adc': {"setup":(o,p)=>"", 
          "loop":(o,p)=>`inputs[${p[0].loop}]`},
  ...
  'sah':{"setup":(o,p)=>`${o} = new Module.maxiSampleAndHold();`, 
