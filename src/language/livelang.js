...
var grammar = {
    ...
    {"name": "Expression", "symbols": [(lexer.has("variable") ? {type: "variable"} : variable), (lexer.has("paramBegin") ? {type: "paramBegin"} : paramBegin), "Params", (lexer.has("paramEnd") ? {type: "paramEnd"} : paramEnd), (lexer.has("sample") ? {type: "sample"} : sample)], "postprocess": d => ({"@setvar": {"@varname":d[0],"@varvalue":{ "@synth": {"@params":[{"@string":d[4].value.substr(1)}].concat(d[2]), "@jsfunc":{value:"sampler"}, "paramBegin":d[1], "paramEnd":d[3]}}}} )},
    ...
    {"name": "Params$subexpression$2", "symbols": [(lexer.has("variable") ? {type: "variable"} : variable)]},
    {"name": "Params", "symbols": ["Params$subexpression$2"], "postprocess": (d) => ([{"@getvar":d[0][0]}])},
    {"name": "Params$subexpression$3", "symbols": [(lexer.has("variable") ? {type: "variable"} : variable)]},
    {"name": "Params", "symbols": ["Params$subexpression$3", (lexer.has("separator") ? {type: "separator"} : separator), "Params"], "postprocess": (d) => ([{"@getvar":d[0][0]}].concat(d[2]))},
